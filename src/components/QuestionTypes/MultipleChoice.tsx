  ...
  return (
    <div className="space-y-3">
      {question.options.map((option, index) => (
        <button
          onClick={() => onAnswerChange({ selected_option: index })}
          key={index}
          disabled={false}
          className={`w-full rounded-lg border p-4 text-left transition-all ${
            selectedAnswer?.selected_option == index
          ...
          <div className="flex items-start">
            <div
              className={`mr-3 h-5 w-5 flex-shrink-0 rounded-full border ${
                selectedAnswer?.selected_option == index ? 'border-primary-500 bg-primary-500' : 'border-gray-300'
              } mt-0.5 flex items-center justify-center`}
            >
              {selectedAnswer?.selected_option == index && <div className="h-2 w-2 rounded-full bg-white"></div>}
            </div>
            <span>{option}</span>
          </div>
        </button>
